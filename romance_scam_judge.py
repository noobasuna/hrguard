...
def normalize_model_id(model_id: str) -> str:
    """Normalize model ids for same-model checks."""
    value = (model_id or "").strip().lower()
    if "/" in value:
        value = value.split("/", 1)[1]
    for token in ("-instruct", "-it", "-chat", "-thinking"):
        if value.endswith(token):
            value = value[: -len(token)]
    return value.replace("_", "-").replace(".", "-")


def assert_distinct_local_models(generation_model_id: str, judge_model: str) -> None:
    """Require a different local judge than the generation model."""
    if not generation_model_id or not judge_model:
        return
    gen = normalize_model_id(generation_model_id)
    judge = normalize_model_id(judge_model)
    if not gen or not judge:
        return
    if gen == judge or gen in judge or judge in gen:
        raise SystemExit(
            "Local judging requires a judge model different from the generation model. "
            f"generation_model_id={generation_model_id!r} judge_model={judge_model!r}"
        )
...
def make_api_client(transport: str) -> OpenAI:
    ...
    if transport == "moonshot":
        ...
        if not api_key:
            raise SystemExit("MOONSHOT_API_KEY is not set and no .env file was found")
        base_url = os.getenv("MOONSHOT_BASE_URL", "https://api.moonshot.ai/v1")
        return OpenAI(api_key=api_key, base_url=base_url)
    raise ValueError(f"Unsupported API transport: {transport}")


def make_vllm_client(base_url: str, api_key: str = "EMPTY") -> OpenAI:
    return OpenAI(api_key=api_key or "EMPTY", base_url=base_url.rstrip("/"))
...
def extract_generated_text(record: Dict[str, Any]) -> str:
    for key in (
        "generated_text",
        "generated text",
        "raw_final_response",  # rebuilt ungated finals (EMPTY-fix path)
        "final_response",
        "response",
        "output",
        "assistant",
    ...
    if isinstance(turn_outputs, list) and turn_outputs:
        last_turn = turn_outputs[-1]
...
def parse_judge_content(content: str, mode: str) -> Dict[str, Any]:
...
def build_local_judge_prompt(record: Dict[str, Any], generated_text: str, mode: str, condition: str) -> str:
    return LOCAL_JUDGE_PROMPT_TEMPLATE.format(
        mode=mode,
        condition=condition,
...
def judge_record(
    ...
    if transport in {"openclaw-local", "vllm-local"}:
        judge_prompt = build_local_judge_prompt(record, generated_text, mode, condition)
    ...
    return parse_judge_content(content, mode)
...
def build_output_row(
    ...
    record_mode = record.get("mode", default_mode)
    if record_mode not in {"attacker", "victim"}:
        record_mode = default_mode
    return {
        ...
        "generation_model_id": generation_model_id or record.get("generation_model_id"),
        "judge": judged,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=["openclaw-local", "vllm-local", "openai", "deepseek", "moonshot"],
        help="How to call the judge model",
    )
    parser.add_argument(
        "--judge-base-url",
        default=os.getenv("JUDGE_BASE_URL", "http://127.0.0.1:8001/v1"),
        help="OpenAI-compatible base URL for vllm-local transport",
    )
    parser.add_argument(
        "--generation-model-id",
        default=os.getenv("GENERATION_MODEL_ID", ""),
        help="Generation model id; required to enforce distinct local judge model",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Parallel workers for openclaw-local / vllm-local transports",
    )
    ...
    if args.transport in {"openclaw-local", "vllm-local"}:
        assert_distinct_local_models(args.generation_model_id, args.model)

    client: Optional[OpenAI] = None
    if args.transport in {"openai", "deepseek", "moonshot"}:
        client = make_api_client(args.transport)
    elif args.transport == "vllm-local":
        client = make_vllm_client(args.judge_base_url)
    ...
    for file_path in files:
        for record in load_records(file_path):
            ...
            if args.resume and record_id is not None and str(record_id) in completed_ids:
                continue
    ...
    if not records and args.resume:
        print(f"No new records to judge. Existing output: {output_path}")
        return

    output_mode = "a" if args.resume and output_path.exists() else "w"
    total = 0
    with output_path.open(output_mode, encoding="utf-8") as fout:
        local_parallel = args.transport in {"openclaw-local", "vllm-local"} and args.workers > 1
        if local_parallel:
            ...
            for item in records:
                ...
                fout.flush()
                total += 1
                if total % 10 == 0 or total == len(records):
                    print(f"Completed {total}/{len(records)} records", flush=True)
                if args.limit and total >= args.limit:
                    print(f"Stopped after {total} records due to --limit")
                    print(f"Wrote: {output_path}")
                    return

    print(f"Wrote {total} judged records to {output_path}", flush=True)
